...
_ALLOWED_INTENTS = {"general", "time"}

def build_supervisor_graph(*, settings: AppSettings, client_factory: Callable[[], ResponsesClient]):
    def classify_request(state: SupervisorState) -> SupervisorState:
        ...
        raw_intent = (response.output_text or "").strip().casefold()
        normalized = raw_intent.strip(" .,:;!?`'\"")
        if normalized not in _ALLOWED_INTENTS:
            raise RuntimeError(f"Foundry router returned an unsupported intent: {raw_intent or '<empty>'}.")
        intent: Intent = normalized  # type: ignore[assignment]
        return {"user_input": user_input, "intent": intent, "answer": ""}
    ...
    def call_foundry_model(state: SupervisorState) -> SupervisorState:
        ...
        return {"user_input": state["user_input"], "intent": state["intent"], "answer": answer}
    ...
    def get_current_utc_time(state: SupervisorState) -> SupervisorState:
        ...
        return {
            ...
            "intent": state["intent"],
            "answer": f"The current UTC time is {current_time.strftime('%Y-%m-%d %H:%M:%S')} UTC.",
        }
    ...
    graph.add_conditional_edges(
        "classify_request",
        route_request,
        {"general": "call_foundry_model", "time": "get_current_utc_time"},
    )
